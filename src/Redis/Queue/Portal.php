...
class Portal
{
    ...
    private function spawn($callable, Caster $job)
    {
        ...
        switch ($pid) {
            ...
            case 0:
                try {
                    set_time_limit($invoker->get('ttr'));
                    $job->reserve();
     ...
     *
     * @param string $keyChip
     * @param integer $timePort
     * @throws Exception
     */
    private function sweeper($keyChip, $timePort)
    {
        $page = 1000;
        $key = $keyChip . Job::STATE_RESERVED;
        ...
        for ($i = 0; $i < $batchNum; $i++) {
            ...
            if (is_array($ret)) {
                foreach ($ret as $id) {
                    ...
                    $ttr = $job->get('ttr') * 1000;
                    if ((Util::now() - $job->get('updated_at')) > $ttr) {
                        $job->error('More than the processing time');
                        Caster::reload($id)->bury();
                    }
                }
            }
        }
