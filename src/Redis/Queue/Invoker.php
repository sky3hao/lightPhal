...
class Invoker extends Job
{
     ...
     * @param $seconds
     */
    public function ttr($seconds)
    {
        $this->injectors['ttr'] = $seconds;
    }
    ...
    private function update()
    {
        ...
        $rs = $this->client->hMset($this->getJobKey(), $job);
        if ($rs === false) {
            return false;
        }
    ...
    public function attempt($fn)
    {
        ...
        $attempts = $this->client->hIncrBy($this->getJobKey(), 'attempts', 1);
        $fn($max - $attempts);
    }
    ...
    public function error($error = null)
    {
        ...
        if ($this->queue->distributed) {
            return $this->client->del($this->getJobKey() . ':lock');
        }
